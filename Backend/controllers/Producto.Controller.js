...
exports.getAllProductos = async (req, res) => {
  ...
  try {
    const productos = await Producto.findAll({
      where: whereClause,
      order: [['id', 'ASC']] // Ordena por la columna 'id' en orden ascendente (ASC)
    })
    if (!productos.length) {
    ...
    const productosConImagenes = productos.map((producto) => {
      ...
      if (producto.imagen) {
        imagenDescomprimida = `data:image/webp;base64,${producto.imagen.toString(
          'base64'
        )}`
      }
      return {
    ...
    res
      .status(200)
      .json({
        message: 'Productos retornados con exito',
