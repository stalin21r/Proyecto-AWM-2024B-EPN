...
exports.getTurnos = async (req, res) => {
  try {
    const turnos = await sequelize.query('SELECT * FROM public.vista_turnos', {
    ...
    res.status(200).json({ message: 'Turnos obtenidos correctamente.', turnos })
  } catch (error) {
    res
      .status(500)
...
exports.getTurnoById = async (req, res) => {
  ...
  try {
    ...
    if (!turno) {
      return res
        .status(404)
        .json({ message: 'Turno no encontrado.', error: '404 Not Found' })
    }
